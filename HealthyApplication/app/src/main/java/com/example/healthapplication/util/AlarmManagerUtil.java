...
public class AlarmManagerUtil {
    ...
    public static void setAlarm(Context context, int flag, int hour, int minute, int id, int
        ...
        Intent intent = new Intent("com.example.healthapplication.ALARM");
        if(Build.VERSION.SDK_INT >= 26){
            ComponentName componentName=new ComponentName(context.getApplicationContext(),"com.example.healthapplication.AlarmReceiver");//参数1-包名 参数2-广播接收者所在的路径名
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            am.set(AlarmManager.ELAPSED_REALTIME_WAKEUP, SystemClock.elapsedRealtime()+10*1000,
                    sender);
            Log.d("alarm","am.if");
            ...
            am.setRepeating(AlarmManager.RTC_WAKEUP, calMethod(week, calendar.getTimeInMillis
                    ...
                    ()), intervalMillis, sender);
            am.set(AlarmManager.ELAPSED_REALTIME_WAKEUP, System.currentTimeMillis()+10*1000, sender);
